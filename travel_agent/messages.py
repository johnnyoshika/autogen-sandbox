...
workflow_graph = '`data_retriever`->`user_proxy`->`analyst`->`user_proxy`->`travel_agent`->`senior_analyst`'
...
system_message_analyst = f'''
    ...
    Follow this framework when writing your Queries:
...
system_message_senior_analyst = f'''
    Role: `senior_analyst`
    ...
    Here is the workflow graph for your reference:
    {workflow_graph}
